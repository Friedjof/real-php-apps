...
if($f=opendir(KAL_Pfad.'temp')){
 ...
 closedir($f);
 if($n=count($aLsch)) $Msg.='<p class="admMeld">'.$n.' Dateien aus dem Ordner <i>temp/</i> gel�scht.</p>';
...
if($f=opendir(KAL_Pfad.KAL_CaptchaPfad)){ // captcha bereinigen
 ...
 closedir($f);
 if($n=count($aLsch)) $Msg.='<p class="admMeld">'.$n.' Dateien aus dem Ordner <i>'.KAL_CaptchaPfad.'</i> gel�scht.</p>';
 foreach($aLsch as $s) @unlink(KAL_Pfad.KAL_CaptchaPfad.$s);
}
if(!$Msg) $Msg='<p class="admMeld">Keine Dateien aus den Ordnern <i>'.KAL_CaptchaPfad.'</i> bzw. <i>temp/</i> zu l�schen.</p>';

...
echo fSeitenKopf('Termin eingeben','<script type="text/javascript">
...
$nFelder=count($kal_FeldName); $bOK=false; $sFehl=''; $sZ=''; $sF=''; $sPCode=''; $sPeri=''; $sOnl='1';
$M1Day=''; $M2Day=''; $M3Day=''; $M4Day=''; $M5Day=''; $M6Day=''; $M7Day=''; $M8Day='';$M9Day=''; $WdhDat=''; $WdhMal='';
$aFehl=array(); $aW=array(); $aOh=array(); $aWTag=array(); $aWT14=array(); $aIds=array(); $bMitBild=false;

if($_SERVER['REQUEST_METHOD']!='POST'){ //GET
 if($p=array_search('u',$kal_FeldType)) $aW[$p]='0000'; $aVg=@file(KAL_Pfad.KAL_Daten.KAL_Vorgaben);
 for($i=1;$i<$nFelder;$i++){
  switch($kal_FeldType[$i]){
  case 't': if(isset($aVg[$i])){$a=explode(';',trim($aVg[$i])); if(isset($a[1])) $aW[$i]=trim($a[1]);} break;
  case 'm': if(isset($aVg[$i])){$a=explode(';',trim($aVg[$i])); if(isset($a[1])) $aW[$i]=str_replace('\n ',"\n",trim($a[1]));} break;
  case 'j': case 'v': if(isset($aVg[$i])){$a=explode(';',trim($aVg[$i])); if(isset($a[1])) $aW[$i]=strtoupper(substr(trim($a[1]),0,1));} break;
  case '@': if($kal_FeldName[$i]!='ZUSAGE_BIS') $aW[$i]=fKalAnzeigeDatum(date('Y-m-d')).date(' H:i'); break;
  }
 }
}else{ //POST Formularauswertung
 $bUtf8=((isset($_POST['kal_JSSend'])||$_POST['kal_Utf8']=='1')?true:false);
 $sOnl=(isset($_POST['kal_Onl'])?$_POST['kal_Onl']:'1'); $nDtPos2=0;
 for($i=1;$i<$nFelder;$i++) if($kal_FeldType[$i]=='b'||$kal_FeldType[$i]=='f')
  $aOh[$i]=(isset($_POST['kal_Oh'.$i])?$_POST['kal_Oh'.$i]:''); //kal_Oh: hochgeladene;
 for($i=1;$i<$nFelder;$i++){
  ...
  if(strlen($s)>0||!$kal_PflichtFeld[$i]||$t=='b'||$t=='f'||$t=='@'){
   ...
   case 'm': case 'a': case 'k': case 's': case 'j': case '#': case 'v': case 'g': case 'u': case 'x': //Memo,Kategorie,Auswahl,Ja/Nein,Nutzer,StreetMap
    break;
   case 'd': //Datum
    if($s) if($v=fKalErzeugeDatum($s)) $s=fKalAnzeigeDatum($v); else $aFehl[$i]=true;
   ...
   case 'b': //Bild
    ...
    }elseif($s>'') if(isset($_POST['kal_Dl'.$i])&&$_POST['kal_Dl'.$i]){ //hochgeladenes Bild l�schen
   ...
   case 'f': //Datei
    ...
    }elseif($s>'') if(isset($_POST['kal_Dl'.$i])&&$_POST['kal_Dl'.$i]){ //hochgeladene Datei l�schen
 ...
 if($sFehl==''){ //alles OK, eintragen
  if(count($aFehl)==0){
   ...
   if(!KAL_SQL){ //ohne SQL
    ...
    }else $Msg='<p class="admFehl">'.KAL_TxSqlEinfg.'</p>'.$DbO->error;
   }else $Msg='<p class="admFehl">'.KAL_TxSqlVrbdg.'</p>';//SQL
   if($bOK){ // Daten gespeichert
    $Msg='<p class="admErfo">'.KAL_TxEingabeErfo.'</p>';
    for($i=1;$i<$nFelder;$i++){
...
if(!$Msg) $Msg='<p class="admMeld">'.KAL_TxEingabeMeld.'</p>';
...
<form name="kalEingabe" action="eingabe.php" onsubmit="return formSend()" enctype="multipart/form-data" method="post">
...
<?php
 for($i=1;$i<$nFelder;$i++){
  ...
  $t=$kal_FeldType[$i]; $v=(isset($aW[$i])?$aW[$i]:''); //Feldinhalt
  switch($t){
  ...
  case 'd': //Datum
   ...
   break;
  case '@': //EintragsDatum
   if($kal_FeldName[$i]!='ZUSAGE_BIS') echo $v;
   else echo $sZ.'<input style="width:12em" type="text" name="kal_F'.$i.'" value="'.$v.'" maxlength="16" /> <span class="admMini">'.KAL_TxFormat.' '.fKalDatumsFormat().' '.KAL_TxOder.' '.fKalDatumsFormat().' '.KAL_TxSymbUhr.'</span></div>';
   break;
  case 'z': //Zeit
  ...
  case 'n': case 'r': case '1': case '2': case '3': case 'o': //Zahlen
   ...
   break;
  case 'b': //Bild
   echo $sZ.'<input style="width:99%" type="file" name="kal_Up'.$i.'" size="80" onchange="loadImgFile(this)" accept="image/jpeg, image/png, image/gif" /></div>'; $bMitBild=true;
   ...
   break;
  case 'f': //Datei
   echo $sZ.'<input style="width:99%" type="file" name="kal_Up'.$i.'" size="80" /></div>';
   if($v) echo NL.'   <div style="float:left;"><input class="admCheck" type="checkbox" name="kal_Dl'.$i.'" value="1" /><input type="hidden" name="kal_F'.$i.'" value="'.$v.'" /><input type="hidden" name="kal_Oh'.$i.'" value="'.(isset($aOh[$i])?$aOh[$i]:'').'" /> <span class="admMini">'.$v.' '.KAL_TxLoeschen.'</span></div>';
 ...
 if(KAL_Periodik){
  ...
  <td><div'.(isset($aFehl['D'])&&$aFehl['D']?' class="admFehl"':'').'>'.KAL_TxImmer.' '.KAL_TxAm.'
   <input style="width:1.8em;" type="text" name="kal_M1Day" value="'.$M1Day.'" maxlength="2" onkeyup="fSelWt(this.value,4)" />. '.KAL_TxTag.'  '.KAL_TxUnd.'  '.KAL_TxAm.'
   <input style="width:1.8em;" type="text" name="kal_M2Day" value="'.$M2Day.'" maxlength="2" onkeyup="fSelWt(this.value,4)" />. '.KAL_TxTagDesMonat.'
   in&nbsp;jedem&nbsp;<select style="width:10.5em;" name="kal_M3Day" onchange="fSelWt(this.value,4)"><option value="1"'.($M3Day!=1?'':' selected="selected"').'>Monat</option><option value="2"'.($M3Day!=2?'':' selected="selected"').'>2. Monat</option><option value="3"'.($M3Day!=3?'':' selected="selected"').'>3. Monat</option><option value="4"'.($M3Day!=4?'':' selected="selected"').'>4. Monat</option><option value="6"'.($M3Day!=6?'':' selected="selected"').'>6. Monat</option></select></div>
  </td>
 </tr><tr class="admTabl">
  ...
  echo '</select> '.KAL_TxTagDesMonat.'
   in&nbsp;jedem&nbsp;<select style="width:10.5em;" name="kal_M6Day" onchange="fSelWt(this.value,5)"><option value="1"'.($M6Day!=1?'':' selected="selected"').'>Monat</option><option value="2"'.($M6Day!=2?'':' selected="selected"').'>2. Monat</option><option value="3"'.($M6Day!=3?'':' selected="selected"').'>3. Monat</option><option value="4"'.($M6Day!=4?'':' selected="selected"').'>4. Monat</option><option value="6"'.($M6Day!=6?'':' selected="selected"').'>6. Monat</option></select></div>
  </td>
 </tr><tr class="admTabl">
...
<p class="admSubmit"><?php if(!$bOK){?><input class="admSubmit" type="submit" value="Eintragen"><?php }else{?>[ <a href="eingabe.php">n�chster Termin</a> ]<?php }?></p>
</form>
<?php
if(file_exists('liste.php')) echo '<p class="admSubmit">[ <a href="liste.php">zur�ck zur Liste</a> ]</p>'.NL.NL;

if($bMitBild && KAL_BildResize){
 echo "\n".'<script src="'.$sHttp.'kalEingabeBild.js" type="text/javascript"></script>';
 echo "\n".'<script type="text/javascript">';
 echo "\n".' sPostURL="eingabe.php";';

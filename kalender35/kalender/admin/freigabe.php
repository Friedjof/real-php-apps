...
echo fSeitenKopf('Termine freigeben','<script language="JavaScript" type="text/javascript">
...
<link rel="stylesheet" type="text/css" href="'.KALPFAD.'kalStyles.css">','TTf');
...
if(isset($_GET['kal_Frei'])&&$_GET['kal_Frei']){
 if($sId=(isset($_GET['kal_Num'])?$_GET['kal_Num']:'')){
  ...
  if($bOK){ //Daten gespeichert
   $Msg='<p class="admErfo">Der Terminvorschlag wurde veröffentlicht</p>'; $sMTo=''; $aN=NULL;
...
if(!KAL_SQL){ //Textdaten
 ...
 for($i=1;$i<$nSaetze;$i++){ //über alle Datensaetze
  ...
  if($rR=$DbO->query('SELECT id'.$t.',periodik FROM '.KAL_SqlTabT.' WHERE online="2" ORDER BY id')){
...
if(!$Msg) $Msg='<p class="admMeld">Bearbeiten Sie die vorliegenden Terminvorschläge.</p>';

//Scriptausgabe
echo $Msg.NL;
?>

<form name="TerminListe" action="freigabe.php" method="post">
...
<?php //Kopfzeile
 ...
 foreach($aD as $a){ //Datenzeilen ausgeben
  ...
  echo NL.' <td align="center" valign="top">'.($bAendern?'<a href="aendern.php?kal_Num='.$sId.'&kal_Vmk=1"><img src="'.$sHttp.'grafik/icon_Aendern.gif" width="12" height="13" border="0" title="Bearbeiten"></a>':'&nbsp;').'</td>';
  echo NL.' <td align="center" valign="top"><a href="freigabe.php?kal_Num='.$sId.'&kal_Frei=1"><img src="'.$sHttp.'grafik/icon_Freigabe.gif" width="12" height="13" border="0" title="Freigeben"></a></td>';
  for($j=1;$j<$nSpalten;$j++){
   ...
   if($s=$a[$j]){
    switch($t){
     ...
     case 'd': case '@': $w=trim(substr($s,11)); // Datum
      ...
      if($j==1&&$bDetail) $s='<a href="detail.php?kal_Num='.$sId.'&amp;kal_Vmk=1">'.$s.'</a>';
      break;
     ...
     case 'l':
      ...
      $s='<a href="'.(strpos($s,'@')?'mailto:'.$s:(($p=strpos($s,'tp'))&&strpos($s,'://')>$p||strpos('#'.$s,'tel:')==1?'':'http://').fKalExtLink($s)).'" target="_blank">'.$v.'</a>';
      break;
     case 'e': case 'c':

<?php
include 'hilfsFunktionen.php'; //Zusagenliste drucken
...
$kal_DrckFelder=explode(';',KAL_ZusageDrckAdm); $bMitNr=($kal_DrckFelder[0]>0); $kal_DrckFelder[0]=0; $aDrckFelder=array_flip($kal_DrckFelder);
$aZ=array(); $aT=array(); $sQ=''; $aZusageFeldTyp=explode(';',KAL_ZusageFeldTyp);
...
//Scriptausgabe
if(!$Msg){
 if(!$sQ) $Msg='<p class="admMeld">Terminzusagen/Reservierungen/Bestellungen</p>';
...
for($j=1;$j<=$nZusageFelder;$j++) if(isset($aDrckFelder[$j])&&($i=$aDrckFelder[$j])){
...
foreach($aD as $a){ //Datenzeilen ausgeben
 ...
 for($j=1;$j<=$nZusageFelder;$j++) if(isset($aDrckFelder[$j])&&($i=$aDrckFelder[$j])){
  ...
  switch($i){
   ...
   case 6: $s='<img src="'.$sHttp.'grafik/punkt'.($a[6]=='1'?'Grn':($a[6]=='0'?'Rot':($a[6]=='-'?'RotX':($a[6]=='2'?'RtGn':($a[6]=='*'?'RtGnX':'Rot'))))).'.gif" width="12" height="12" border="0">'; break;
   case 7: $s=sprintf('%0'.KAL_NummerStellen.'d',$a[7]); break;
   case 8: $s=(!KAL_SQL?fKalDecode($a[8]):$a[8]); break;

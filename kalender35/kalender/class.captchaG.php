...
/*--(c) J. Hummel------------------------------------------------------------
 ...
 function Generate($sTCol='#000099',$sBCol='#F0F0F0'){ //Grafik erzeugen und Namen liefern
  ...
  if($Img=imagecreatetruecolor(120,24)){
   ...
   if(imagejpeg($Img,$this->Path.$sNam,80)){
    $this->Question=$sNam;
    if(file_exists($this->KeyFile)) $aC=file($this->KeyFile); else $aC=array(); $sCsv=''; $nTime=time(); $sRefTime=strval($nTime-3600);
 ...
 function Delete(){ //Captcha loeschen
  ...
  if(strlen($sAnswer)==5&&$sPublic==md5($this->Salt.'*'.$sAnswer)){ //richtig
   if(file_exists($this->KeyFile)) $aC=file($this->KeyFile); else $aC=array(); $sCsv=''; $sRefTime=strval(time()-1200);
   if(is_array($aC)&&($nCnt=count($aC))){
    for($i=0;$i<$nCnt;$i++){
     $sLn=rtrim($aC[$i]);

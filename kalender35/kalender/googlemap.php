...
<title>Termineingabe:: Google-Maps - Koordinaten bestimmen</title>
<style type="text/css">
...
?>
<script src="<?php echo KAL_GMapURL?>"></script>
<script>
 var img; var poi; poi=null;
<?php if(KAL_GMapV3){?>
 function showMap(){
  var mapLatLng=new google.maps.LatLng(<?php echo $nBr?>,<?php echo $nLn?>);
  var mapOption={zoom:<?php echo $nZm?>,center:mapLatLng,panControl:true,mapTypeControl:<?php echo(KAL_GMapTypeControl?'true':'false')?>,streetViewControl:false,mapTypeId:google.maps.MapTypeId.ROADMAP};
...
<?php if($nPb!=0||$nPl!=0){?>
  ...
  poi=new google.maps.Marker({position:poiLatLng,map:img,title:'<?php echo iconv('ISO-8859-1','UTF-8',KAL_TxGMapOrt)?>'});
<?php }?>
 ...
 function geocodeAddress(geocoder,img){
  ...
  geocoder.geocode({'address': address}, function(results, status){
   if(status===google.maps.GeocoderStatus.OK){
    ...
    poi=new google.maps.Marker({position:results[0].geometry.location,map:img,title:'<?php echo iconv('ISO-8859-1','UTF-8',KAL_TxGMapOrt)?>'});
 ...
 function placeMrk(location){
  if(poi!=null){poi.setMap(null); poi=null;}
  poi=new google.maps.Marker({position:location,map:img,title:'<?php echo iconv('ISO-8859-1','UTF-8',KAL_TxGMapOrt)?>'});
...
<?php }else{ /* Ver.2 */ ?>
 function showMap(){
  if(GBrowserIsCompatible()){
   ...
   <?php if(KAL_GMapTypeControl) echo 'img.addControl(new GMapTypeControl());'."\n"?>
...
<body onload="showMap()"<?php echo(KAL_GMapV3?'':' onunload="GUnload"')?>>
...
<li>entweder durch Ortseingabe direkt &uuml;ber der Karte</li>
...
<div id="Karte" style="width:98%;max-width:<?php echo KAL_GMapBreit?>px;height:<?php echo KAL_GMapHoch?>px;text-align:center;margin:4px;margin-left:auto;margin-right:auto"></div>
</div>
<form name="geo" action="">
<div style="text-align:center;margin-top:6px;">
<table style="width:98%;margin:4px;margin-left:auto;margin-right:auto;border:0">
<tr>

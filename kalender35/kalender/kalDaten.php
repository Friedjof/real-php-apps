...
function fKalDaten($bListe,$bLimit=true){ //Daten bereitstellen
  ...
  if($sIntervall>'-'){ //Filter ist sichtbar
   ...
   if($sIntervall>'0'){ //filtern
    if($sIntervall<'a'){ //Normalintervall oder Archiv oder alle
     if($sIntervall!='@'&&$sIntervall!='[]'){ //kein Archiv oder Gesamtheit_f�r_InfoDetail
 ...
 define('KAL_IFilter',$sIntervall); /*f�r Filteranzeige in Terminliste*/ if($sIv=='@') $sIntervall='@';
 ...
 if(!KAL_SQL){ //Textdaten
  ...
  for($i=1;$i<$nSaetze;$i++){ //�ber alle Datens�tze
   ...
   if($b){ //Datensatz gueltig
    ...
    if($bAktuSuche||$bLfndSuche){ //aktuelle Ereignisse pr�fen
     ...
     if($bAktuSuche){
      if(KAL_AktuEnde&&$sEDt>=$sJetztDat){
       if(KAL_AktuZeit&&$sEDt==$sJetztDat){ //Endzeit bei heute ber�cksichtigen
        if($sEZt>=$sJetztUhr||$sEZt==''){ //Endzeit sp�ter oder ohne Endzeit
      ...
      if($sADt>=$sJetztDat){ //kommendes Anfangsdatum
       if(KAL_AktuZeit&&$sADt==$sJetztDat){ //Zeit bei heute ber�cksichtigen
     ...
     if($bLfndSuche){
      ...
      }else $bLfndSuche=false; //Anfangsdatum sp�ter
     }//LfndSuche
    }//AktuSuche||LfndSuche
   }//gueltig
  }//$nSaetze
...
function fKalSonderintervall($w){ //Sonderintervall bilden
 ...
 switch($w){
  ...
  case 'c': $n=8-date('w'); if($n>7) $n=1; $n=time()+$n*86400; $s=date('Y-m-d',$n).date('Y-m-d',$n+518400); break; //n�chste Woche
  case 'f': if(++$M>12){$M= 1; $J++;} $s=sprintf('%04d-%02d-01%04d-%02d-31',$J,$M,$J,$M); break; //n�chster Monat
  case 'i': $M-=(($M-1)%3)-3; if($M>12){$M=1; $J++;} $s=sprintf('%04d-%02d-01%04d-%02d-31',$J,$M,$J,$M+2); break; //n�chstes Quartal
  case 'l': $M-=(($M-1)%6)-6; if($M>12){$M=1; $J++;} $s=sprintf('%04d-%02d-01%04d-%02d-31',$J,$M,$J,$M+5); break; //n�chstes Halbjahr
  case 'o': $s=sprintf('%04d-01-01%04d-12-31',++$J,$J); break; //n�chstes Jahr

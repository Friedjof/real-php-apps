...
function fKalSeite(){ //Seiteninhalt
 global $kal_FeldName, $kal_FeldType, $kal_LinkFeld, $kal_Symbole, $kal_WochenTag,
  $aMonate, $sSes, $sMon, $sWoc, $aNutzer, $nNutzerZahl, $nId, $ksNutzerListFeld;
 ...
 if(KAL_MonWochNr) $X.="\n".'  <div class="kalTb'.(KAL_DruckMFarbig?'SpW':'Dr" style="vertical-align:top').'">'.str_replace('#W',date('W',$nZt),KAL_TxMWochNr).'</div>';

 for($i=1;$i<=7;$i++){ //ueber alle 7 Tage
  $sD=date('Y-m-d',$nZt); $s2=substr($sD,5,2); $s3=substr($sD,0,4); $s1=substr($sD,8,2); $nD=(int)$s1; $nM=(int)$s2; $nJ=(int)$s3; $nZt+=86400;
  $sTrm=''; $sHgDet=''; $sHgDat=''; $sCss='Dat'; if($nD==$nHtT&&$nM==$nHtM&&$nJ==$nHtJ) $sCss='Hte'; //heute?
  $sDtTpl=fKalDtTempl($s2,$s3); $sDt=str_replace('##',sprintf('%02d',$s1),$sDtTpl); //anzuzeigendes Datum
  if($aTag[$nD]){ //Datum ist besetzt
   $nTId=$aTag[$nD][0]; $nTrms=count($aTag[$nD]); $sMehr='';

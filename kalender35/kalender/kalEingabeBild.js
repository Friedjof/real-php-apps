/* Bilder vor dem Upload vor-verkleinern */
 ...
 function loadDatFile(inputField){
  nFldNr=parseInt(inputField.name.substr(6));
 ...
 function resizeImg(img,exifOrientation){
  ...
  if(canvas.width>0 && canvas.getContext){
   ...
   canvas.toBlob(function(blob){ // Kontrollausgabe als Thumbnail
    ...
    newImg.src=url;
    var oLabel=document.getElementById("kalLabel"+nFldNr);
 ...
 function formSend(){
  if(bJSSend){
   ...
   formData.append("kal_JSSend","1");
   for(var j=0;document.kalEingabe.elements.length>j;j++){
    if(document.kalEingabe.elements[j].type=="checkbox"||document.kalEingabe.elements[j].type=="radio"){
     if(document.kalEingabe.elements[j].checked) formData.append(document.kalEingabe.elements[j].name,document.kalEingabe.elements[j].value);
    }else if(document.kalEingabe.elements[j].type!="file"){
     if(document.kalEingabe.elements[j].value>"") formData.append(document.kalEingabe.elements[j].name,document.kalEingabe.elements[j].value);
    }else if(document.kalEingabe.elements[j].value){ // Typ: file
     nFldNr=parseInt(document.kalEingabe.elements[j].name.substr(6));
     formData.append("kal_UpNa_"+nFldNr,document.kalEingabe.elements[j].value);
     formData.append(document.kalEingabe.elements[j].name,blobs[nFldNr],document.kalEingabe.elements[j].value);
   }}

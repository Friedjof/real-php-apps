...
function fKalSeite(){ //Seiteninhalt
 ...
 $Et=''; $Es='Fehl'; $aR=NULL; $H=''; $U=''; $sEml=''; $sTrm=''; $sTm2=''; $sHid=''; $sId=''; $sNutzerEml='';
 $bEml=true; $bTrm=true; $bOK=true; $bDo=true; $bZusageBis=false; $sZusageBis=''; $bDSE1=false; $bDSE2=false; $bErrDSE1=false; $bErrDSE2=false;
 ...
 if($_SERVER['REQUEST_METHOD']=='POST'){ //POST
  if(isset($_POST['kal_Nummer'])){
   $sId=sprintf('%0d',$_POST['kal_Nummer']); reset($_POST); if(KAL_ErinnernDSE1) $bErrDSE1=true; if(KAL_ErinnernDSE2) $bErrDSE2=true;
   foreach($_POST as $k=>$v) if(substr($k,0,4)=='kal_'&&substr($k,4,1)!='e'&&substr($k,4,3)!='Cap'&&substr($k,4,3)!='DSE'){
    ...
    if(strlen($s)>0){
     if($k=='kal_eEml') $sEml=$s;
     elseif($k=='kal_eTrm') $sTrm=sprintf('%0d',(int)$s);
     elseif($k=='kal_eTm2') $sTm2=sprintf('%0d',(int)$s);
     elseif($k=='kal_DSE1'){if($s=='1'){$bDSE1=true; $bErrDSE1=false;}}
     elseif($k=='kal_DSE2'){if($s=='1'){$bDSE2=true; $bErrDSE2=false;}}
    }
   }
   if(strlen($sEml)==0||!preg_match('/^([0-9a-z~_-]+\.)*[0-9a-z~_-]+@[0-9a-zäöü_-]+(\.[0-9a-zäöü_-]+)*\.[a-z]{2,16}$/',strtolower($sEml))){$bEml=false; $bOK=false;}
   if(strlen($sTm2)>0) $sTrm=''; elseif(strlen($sTrm)==0){$bTrm=false; $bOK=false;}
 ...
 if($sId>''){//Terminnummer
  ...
  if(is_array($aR)&&count($aR)>2){ //Termine vorhanden
   ...
   if(KAL_ErinnNDetail) $kal_DetailFeld=$kal_NDetailFeld;
   $H.="\n".'<div class="kalTabl">';
   for($i=1;$i<$nFelder;$i++){
    $t=$kal_FeldType[$i]; $sFN=$kal_FeldName[$i]; $u='';
    if($sFN=='ZUSAGE_BIS') if($sZusageBis=$aR[$i]) if($sZusageBis>=date('Y-m-d H:i')) $bZusageBis=true;
    if($kal_DetailFeld[$i]>0&&$t!='p'&&$t!='c'&&substr($sFN,0,5)!='META-'&&$sFN!='TITLE'){
     if($s=$aR[$i]){$u=$s;
      switch($t){
       case 't': $s=fKalBB(fKalDt($s)); $u=@strip_tags($s); break; //Text/Memo
       case 'm': if(KAL_ErinnMitMemo){$s=fKalBB(fKalDt($s)); $u=@strip_tags($s);} else{$s=''; $u='';} break; //Memo
         ...
         if(KAL_NutzerErinnFeld>0){
          ...
          if(!KAL_SQL){ //Textdaten
           ...
           for($j=1;$j<$nSaetze;$j++) if(substr($aD[$j],0,$p)==$v){
            ...
            if(!$s=$aN[KAL_NutzerErinnFeld]) $s=KAL_TxAutorUnbekannt; elseif(KAL_NutzerErinnFeld<5&&KAL_NutzerErinnFeld>1) $s=fKalDeCode($s); $s=fKalDt($s);
            break;
           }
          }elseif($DbO){ //SQL-Daten
           if($rR=$DbO->query('SELECT * FROM '.KAL_SqlTabN.' WHERE nr="'.$nId.'"')){
            $aN=$rR->fetch_row(); $rR->close();
            if(is_array($aN)){array_splice($aN,1,1); if(!$s=fKalDt($aN[KAL_NutzerErinnFeld])) $s=KAL_TxAutorUnbekannt;}
   ...
   if($_SERVER['REQUEST_METHOD']=='POST'){
    if($bOK&&!empty($sEml)&&(strlen($sTrm)>0||strlen($sTm2)>0)){//eintragen
     if($sEml==$sNutzerEml||fKalEmlBekannt($sEml,$DbO)){//eintragen
      if($sTm2!=''&&$sZusageBis!=''){
       $sDat=date('Y-m-d',@mktime(12,0,0,substr($sZusageBis,5,2),substr($sZusageBis,8,2),substr($sZusageBis,0,4))-(int)$sTm2*86400);
      }elseif($sTrm!='') $sDat=date('Y-m-d',@mktime(12,0,0,substr($aR[1],5,2),substr($aR[1],8,2),substr($aR[1],0,4))-(int)$sTrm*86400);
      if($sDat>date('Y-m-d')){
       if(!KAL_SQL){
        ...
        }else if($Et=='') $Et=str_replace('#',KAL_Daten.KAL_Erinner,KAL_TxDateiRechte);
       }elseif($DbO){ //SQL
        if($DbO->query('INSERT IGNORE INTO '.KAL_SqlTabE.' (datum,termin,email) VALUES("'.$sDat.'","'.$sId.'","'.$sEml.'")')) $bDo=false;
        else if($Et=='') $Et=KAL_TxSqlEinfg;
       }
       if(!$bDo){
        $Es='Erfo'; $Et=KAL_TxErinnErfo;
        ...
        if(KAL_ErinnOkMail){
         ...
         $Mailer->Subject=str_replace('#',$sWww,str_replace('#A',$sWww,KAL_TxErinnOkBtr));
         $Mailer->Text=str_replace('#D',$U,str_replace('#A',$sWww,str_replace('#T',fKalAnzeigeDatum($sDat),str_replace('\n ',"\n",KAL_TxErinnOkTxt))));
         $Mailer->Send();
        }
       }
      }else $Et=KAL_TxErinnVorbei;
      ...
      if(KAL_FreischaltNeuMail){
       if(isset($_POST['kal_NeuAdr'])&&$_POST['kal_NeuAdr']>'0'){
        ...
        if(!KAL_SQL){
         ...
         if(!$DbO->query('INSERT IGNORE INTO '.KAL_SqlTabM.' (email) VALUES("'.$sCod.';'.$sEml.'")')){$bOK=false; if($Et=='') $Et=KAL_TxSqlEinfg;}
        }
        if($bOK){
         ...
         $Mailer->Subject=str_replace('#',$sWww,str_replace('#A',$sWww,KAL_TxErinnUnbkBtr)); $Mailer->Send();
        }
       }else{
        $Et=str_replace('#',$sEml,KAL_TxErinnUnbekannt); $sHid.='<input type="hidden" name="kal_NeuAdr" value="1">';
       }
      }else $Et=str_replace('#',$sEml,KAL_TxErinnUnmoegl);
     }
    }else $Et=KAL_TxEingabeFehl;
   }else{//GET
    $Et=KAL_TxErinnMeld; $Es='Meld';
...
<script>
 ...
 $X.="\n".'<div class="kalTabl">';
 $X.="\n".'<div class="kalTbZl1"><div class="kalTbSp1">'.fKalTx(KAL_TxEmpfaenger).'<br>'.fKalTx(KAL_TxMailAdresse).'</div><div class="kalTbSp2"><div class="kal'.($bEml?'Eing':'Fhlt').'"><input class="kalEing" type="text" name="kal_eEml" value="'.fKalTx($sEml).'"></div></div></div>';
 $X.="\n".'<div class="kalTbZl2"><div class="kalTbSp1">'.fKalTx(KAL_TxErinnZeit).'</div><div class="kalTbSp2"><div class="kal'.($bTrm?'Eing':'Fhlt').'"><select class="kalEing kalAuto" name="kal_eTrm"><option value="">--</option><option value="0"'.($sTrm=='0'?' selected="selected"':'').'>0 '.fKalTx(KAL_TxTage).'</option><option value="1"'.($sTrm=='1'?' selected="selected"':'').'>1 '.fKalTx(KAL_TxTage).'</option><option value="2"'.($sTrm=='2'?' selected="selected"':'').'>2 '.fKalTx(KAL_TxTage).'</option><option value="3"'.($sTrm=='3'?' selected="selected"':'').'>3 '.fKalTx(KAL_TxTage).'</option><option value="5"'.($sTrm=='5'?' selected="selected"':'').'>5 '.fKalTx(KAL_TxTage).'</option><option value="7"'.($sTrm=='7'?' selected="selected"':'').'>7 '.fKalTx(KAL_TxTage).'</option><option value="10"'.($sTrm=='10'?' selected="selected"':'').'>10 '.fKalTx(KAL_TxTage).'</option><option value="14"'.($sTrm=='14'?' selected="selected"':'').'>14 '.fKalTx(KAL_TxTage).'</option><option value="20"'.($sTrm=='20'?' selected="selected"':'').'>20 '.fKalTx(KAL_TxTage).'</option><option value="30"'.($sTrm=='30'?' selected="selected"':'').'>30 '.fKalTx(KAL_TxTage).'</option></select> '.fKalTx(KAL_TxVorBeginn).'</div>'.($bZusageBis?'<div class="kalNorm"><select class="kalEing" name="kal_eTm2"><option value="">--</option><option value="0"'.($sTm2=='0'?' selected="selected"':'').'>0 '.fKalTx(KAL_TxTage).'</option><option value="1"'.($sTm2=='1'?' selected="selected"':'').'>1 '.fKalTx(KAL_TxTage).'</option><option value="2"'.($sTm2=='2'?' selected="selected"':'').'>2 '.fKalTx(KAL_TxTage).'</option><option value="3"'.($sTm2=='3'?' selected="selected"':'').'>3 '.fKalTx(KAL_TxTage).'</option><option value="5"'.($sTm2=='5'?' selected="selected"':'').'>5 '.fKalTx(KAL_TxTage).'</option><option value="7"'.($sTm2=='7'?' selected="selected"':'').'>7 '.fKalTx(KAL_TxTage).'</option><option value="10"'.($sTm2=='10'?' selected="selected"':'').'>10 '.fKalTx(KAL_TxTage).'</option><option value="14"'.($sTm2=='14'?' selected="selected"':'').'>14 '.fKalTx(KAL_TxTage).'</option><option value="20"'.($sTm2=='20'?' selected="selected"':'').'>20 '.fKalTx(KAL_TxTage).'</option><option value="30"'.($sTm2=='30'?' selected="selected"':'').'>30 '.fKalTx(KAL_TxTage).'</option></select> '.fKalTx('vor dem Anmeldeschluss').'</div>':'').'</div></div>';
 $X.="\n".'<div class="kalTbZl1"><div class="kalTbSp1">'.fKalTx(KAL_TxDetail).'</div><div class="kalTbSp2">'.$H.'</div></div>';
 if(KAL_ErinnernDSE1) $X.="\n".'<div class="kalTbZl2"><div class="kalTbSp1 kalTbSpR">*</div><div class="kalTbSp2"><div class="kal'.($bErrDSE1?'Fhlt':'Eing').'">'.fKalDSEFld(1,$bDSE1).'</div></div></div>';
 if(KAL_ErinnernDSE2) $X.="\n".'<div class="kalTbZl2"><div class="kalTbSp1 kalTbSpR">*</div><div class="kalTbSp2"><div class="kal'.($bErrDSE2?'Fhlt':'Eing').'">'.fKalDSEFld(2,$bDSE2).'</div></div></div>';
 if($bCaptcha){ //Captcha-Zeile
  $X.="\n".' <div class="kalTbZl'.(KAL_ErinnernDSE1||KAL_ErinnernDSE2?'1':'2').'">

<?php
function fKalSeite(){
 global $kal_FeldName, $kal_FeldType, $kal_EingabeFeld, $kal_EingabeLang, $kal_PflichtFeld, $kal_KopierFeld,
  $kal_Kategorien, $kal_Symbole, $kal_WochenTag;

 $nFelder=count($kal_FeldName); $aW=array(); $aWTag=array(); $aWT14=array(); $aUpl=array(); $aOh=array(); $aOa=array(); $aOs=array(); $aFehl=array(); $bOK=false; $bDSE1=false; $bDSE2=false; $bErrDSE1=false; $bErrDSE2=false;
 $Et=''; $Es='Fehl'; $sPeri=''; $sWdhDat=''; $sWdhMal=''; $M1Day=NULL; $M2Day=NULL; $M3Day=NULL; $M4Day=NULL; $M5Day=NULL; $M6Day=NULL; $M7Day=NULL; $M8Day=NULL; $M9Day=NULL; $sSta='1';
 ...
 if($bSesOK) if(KAL_NEingabeAnders){$kal_EingabeFeld=$GLOBALS['kal_NEingabeFeld']; $kal_KopierFeld=$GLOBALS['kal_NKopierFeld'];}

 if($nNuPos>1&&KAL_NEingabeLogin){ //Login zwingend noetig
 ...
 if($_SERVER['REQUEST_METHOD']!='POST'){ //GET - Datensatz bereitstellen
  if($sAltNr=(isset($_GET['kal_Nummer'])?sprintf('%0d',$_GET['kal_Nummer']):'')){ //Datensatznummer vorhanden
   if(!KAL_SQL){ //Textdaten holen
    $aD=file(KAL_Pfad.KAL_Daten.KAL_Termine); $nSaetze=count($aD); $s=$sAltNr.';'; $p=strlen($s);
    for($i=1;$i<$nSaetze;$i++){
     if(substr($aD[$i],0,$p)==$s){$aW=explode(';',str_replace('\n ',"\n",rtrim($aD[$i]))); break;}
    }
   }elseif($DbO){ //SQL-Daten
    if($rR=$DbO->query('SELECT * FROM '.KAL_SqlTabT.' WHERE id="'.$sAltNr.'"')){
     if(!$aW=$rR->fetch_row()) $aW=array(); $rR->close();
    }else $Et=KAL_TxSqlFrage;
   }else $Et=$SqE;
   if(count($aW)>0){ //vorhanden
    $sSta=$aW[1]; array_splice($aW,1,1); $aW[1]=''; $nDatFeld2=0;
    for($i=2;$i<$nFelder;$i++) if($kal_KopierFeld[$i]>0){
     if(!KAL_NKopiereEigene||!$bSesOK||$aW[$nNuPos]==sprintf('%04d',substr(KAL_Session,17,4))){
      ...
      switch($kal_FeldType[$i]){
       case 'd':
        if(KAL_EndeDatum&&$nDatFeld2==0){$nDatFeld2=$i; $aW[$i]='';} //2. Datum suchen
        elseif($aW[$i]) $aW[$i]=fKalAnzeigeDatum($aW[$i]); break;
       ...
       case 'e': case 'c': case 'p': $aW[$i]=''; break;
       case '@': if($kal_FeldName[$i]!='ZUSAGE_BIS') $aW[$i]=trim(fKalAnzeigeDatum(date('Y-m-d')).date(' H:i')); elseif($aW[$i]) $aW[$i]=trim(fKalAnzeigeDatum($aW[$i]).strstr($aW[$i],' ')); break;
     }}else{$aW[$i]=''; if($Et=='') $Et=KAL_TxNummerFremd;}
    }else $aW[$i]='';
  ...
  }else{$Et=KAL_TxNummerFehlt; $Es='Meld';}//keine Datensatznummer
  if(KAL_NutzerEMail){if(!$p=array_search('c',$kal_FeldType)) $p=array_search('e',$kal_FeldType); if($p) $aW[$p]=$sNutzerEml;} //NutzerEMail
  ...
  $sZ=''; $sF=''; $sPCode=''; $sFehl=''; $sSta=(isset($_POST['kal_Sta'])?sprintf('%0d',$_POST['kal_Sta']):(KAL_Direkteintrag?'1':'2')); $sAltNr=(isset($_POST['kal_Nummer'])?sprintf('%0d',$_POST['kal_Nummer']):''); $nDtPos2=0;
  ...
  for($i=1;$i<$nFelder;$i++) if($kal_EingabeFeld[$i]){ //alle Eingabefelder
   ...
   if(strlen($s)>0||!$kal_PflichtFeld[$i]||$t=='b'||$t=='f'||$t=='@'){
    ...
    case 'b': //Bild
     if($aOh[$i]>'') $v=$aOh[$i]; else{if(isset($_POST['kal_Dl'.$i])&&$_POST['kal_Dl'.$i]=='1') $aOa[$i]=''; $v=$aOa[$i];} //kal_Up: neue Datei; kal_Dl: zu löschen
     ...
     $aOs[$i]=$v; break;
    case 'f': //Datei
     if($aOh[$i]>'') $v=$aOh[$i]; else{if(isset($_POST['kal_Dl'.$i])&&$_POST['kal_Dl'.$i]=='1') $aOa[$i]=''; $v=$aOa[$i];}
  ...
  if(KAL_EintragDSE1) if(isset($_POST['kal_DSE1'])&&$_POST['kal_DSE1']=='1') $bDSE1=true; else{$bErrDSE1=true; $aFehl['DSE']=true;}
  if(KAL_EintragDSE2) if(isset($_POST['kal_DSE2'])&&$_POST['kal_DSE2']=='1') $bDSE2=true; else{$bErrDSE2=true; $aFehl['DSE']=true;}
  ...
  if($sPeri=(isset($_POST['kal_Periode'])?@strip_tags($_POST['kal_Periode']):'')){ //Periodik auswerten
   ...
   if($sPeri=='A'){// mehrtaegig
    ...
    if($M1Day=(int)$_POST['kal_M1Day']){if($M1Day<32) $sPCode.=$M1Day; else $aFehl['D']=true;}else{$aFehl['D']=true; $M1Day='';}
    if($M2Day=(int)$_POST['kal_M2Day']){if($M2Day<32) $sPCode.='|'.$M2Day; else $aFehl['D']=true;}else $M2Day='';
    $M3Day=(isset($_POST['kal_M3Day'])?$_POST['kal_M3Day']:''); $sPCode.='|'.$M3Day;
   }elseif($sPeri=='E'){//monatlich-2
    if($M4Day=(int)$_POST['kal_M4Day']) $sPCode.=$M4Day; else{$aFehl['E']=true; $M4Day='';}
    $M5Day=strip_tags($_POST['kal_M5Day']);
    ...
    if($bOK){ //Daten gespeichert
     ...
     for($i=1;$i<$nFelder;$i++) if($kal_EingabeFeld[$i]){
      ...
      if($UpNa=$aOh[$i]){ //neue Bilder und Dateien umspeichern
       ...
       if($t=='b'){
        $p=strpos($UpNa,'|'); $UpNa=substr($UpNa,0,$p);
        reset($aIds); foreach($aIds as $j) if(!@copy(KAL_Pfad.KAL_Bilder.$sAltNr.'-'.$UpNa,KAL_Pfad.KAL_Bilder.$j.'-'.$UpNa)){$bOK=false; $sFehl=$UpNa;}
        $UpNa=substr($aOs[$i],$p+1);
        reset($aIds); foreach($aIds as $j) if(!@copy(KAL_Pfad.KAL_Bilder.$sAltNr.'_'.$UpNa,KAL_Pfad.KAL_Bilder.$j.'_'.$UpNa)){$bOK=false; $sFehl=$UpNa;}
       }elseif($t=='f'){
        reset($aIds); foreach($aIds as $j) if(!@copy(KAL_Pfad.KAL_Bilder.$sAltNr.'~'. $UpNa,KAL_Pfad.KAL_Bilder.$j.'~'.$UpNa)){$bOK=false; $sFehl=$UpNa;}
       }
       if(!$bOK) $Et.=str_replace('#',KAL_Bilder.$sFehl,KAL_TxDateiRechte); $bOK=true;
      ...
      if($t!='c'&&$t!='e'&&$t!='p'){
       $s=($t!='u'?$aW[$i]:$sNutzerName); if($t=='j'&&$s>'') $s=($s!='J'?KAL_TxNein:KAL_TxJa);
...
<script>
 ...
 $X.="\n".'<input type="hidden" name="kal_Aktion" value="kopieren">';
 $X.="\n".'<script>';
 ...
 if(isset($sAltNr)&&$sAltNr>'') $X.="\n".'<input type="hidden" name="kal_Nummer" value="'.$sAltNr.'">';
 $X.="\n".'<div class="kalTabl">';
 ...
 for($i=1;$i<$nFelder;$i++) if($kal_EingabeFeld[$i]){
  ...
  if(array_search($i,$aTrn)){
   $X.="\n".' <div class="kalTbZl'.$nFarb.'"><div class="kalTbSp1">&nbsp;</div><div class="kalTbSp2">&nbsp;</div></div>';
   if(--$nFarb<=0) $nFarb=2;
  }
 }
 ...
 <div class="kalTabl">
  <div class="kalTbZl1">
   ...
   <div class="kalTbSp2"><input class="kalLogi" type="text" name="kal_F2" maxlength="100"></div>
  </div>
  <div class="kalTbZl1">
   <div class="kalTbSp1">'.fKalTx(KAL_TxPasswort).'</div>
   <div class="kalTbSp2"><input class="kalLogi" type="password" name="kal_F3" maxlength="16"></div>
  </div>
 </div>
 <input type="submit" class="kalSchalter" style="background-image:url('.KAL_Url.'grafik/schalter.gif)" value="'.fKalTx(KAL_TxAnmelden).'" title="'.fKalTx(KAL_TxAnmelden).'">';
 ...
 <div class="kalTabl">
  <div class="kalTbZl1">
   ...
   <div class="kalTbSp2"><input class="kalLogi" type="text" name="kal_F2" value="'.(isset($sNeuName)?$sNeuName:'').'" maxlength="25"></div>
  </div>';
 if($bCaptcha){ //Captcha-Zeile
  $X.="\n".' <div class="kalTbZl1">
   <div class="kalTbSp1">'.fKalTx(KAL_TxCaptchaFeld).'*</div>
...
function fKalWdhDat($sBeg,$sCod){
 ...
 $bDo=true; $nAkt=@mktime(12,0,0,substr($sBeg,5,2),substr($sBeg,8,2),substr($sBeg,0,4)); $aDat=NULL; if(!isset($aTmp[4])) $aTmp[4]='0'; $bMon1=false; $bMon11=false;
 while($bDo){
  switch($sTyp){
   case 'A': //mehrtaegig
 ...
 if(is_array($aDat)) return $aDat; else return false;
}

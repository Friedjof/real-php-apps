...
function fKalSeite(){ //Benutzerdaten
 ...
 $aB=array(); $sEml=(isset($a[4])?trim($a[4]):''); //Benachrichtigungen holen
 if($bSes){
  $kal_ListenFeld=$GLOBALS['kal_NListenFeld'];
  $sId=(isset($a[0])?$a[0]:''); $i=1;
  if(!KAL_SQL){
   $aD=file(KAL_Pfad.KAL_Daten.KAL_Benachr); $nSaetze=count($aD); $sE=';'.$sEml;
   for($i=1;$i<$nSaetze;$i++) if($n=strpos($aD[$i],$sE)){
    $s=rtrim($aD[$i]); $aB[$i]=substr($s,0,$n); $aD[$i]=$s."\n";
   }
  }elseif($DbO){ //SQL
   if($rR=$DbO->query('SELECT id,termin FROM '.KAL_SqlTabB.' WHERE email="'.$sEml.'"')){
    while($a=$rR->fetch_row()) if($a[0]>0) $aB[($i++)]=$a[1]; $rR->close();
   }else $Et=KAL_TxSqlFrage;
  }
 }

 $sLsch=''; $aLsch=array();// Benachrichtigungen loeschen
 if($_SERVER['REQUEST_METHOD']=='POST'){
  $Et=KAL_TxNnUnveraendert; $Es='Meld';
   ...
   if(isset($_POST['kal_LschNun'])&&$_POST['kal_LschNun']=='1'){
    if(!KAL_SQL){
     foreach($_POST as $k=>$xx) if(substr($k,0,6)=='kal_L_'){
      $s=substr($k,6,15);
      if($i=array_search(substr($s,strpos($s,'#')+1).';'.$sEml."\n",$aD)){
       $aD[$i]=''; $n++; unset($aB[substr($s,0,strpos($s,'#'))]);
     }}
     if($n){
      if($f=@fopen(KAL_Pfad.KAL_Daten.KAL_Benachr,'w')){ //Benachrichtigungen neu schreiben
       fwrite($f,str_replace("\r",'',rtrim(implode('',$aD)))."\n"); fclose($f);
       $Et=str_replace('#N',$n,KAL_TxNnGeloescht); $Es='Erfo';
      }else{$Et=str_replace('#',KAL_Daten.KAL_Benachr,KAL_TxDateiRechte); $Es='Fehl';}
      $Et=str_replace('#N',$n,KAL_TxNnGeloescht); $Es='Erfo';
     }
    }elseif($DbO){ //SQL
     foreach($_POST as $k=>$xx) if(substr($k,0,6)=='kal_L_'){
      $s=substr($k,6,15);
      if($DbO->query('DELETE FROM '.KAL_SqlTabB.' WHERE termin="'.substr($s,strpos($s,'#')+1).'" AND email="'.$sEml.'" LIMIT 1')){
       if($DbO->affected_rows>0){$n++; unset($aB[substr($s,0,strpos($s,'#'))]);}
      }
     }
     if($n){$Et=str_replace('#N',$n,KAL_TxNnGeloescht); $Es='Erfo';}
    }
   }else{
    foreach($_POST as $k=>$xx) if(substr($k,0,6)=='kal_L_') $aLsch[substr($k,6)]=true;
    if($n=count($aLsch)){$Et=str_replace('#N',$n,KAL_TxNnLoeschen); $Es='Fehl'; $sLsch='1';}
   }
  }
 }//POST
 ...
 }else{$Et=KAL_TxNnKeineBenachr; $Es='Meld';}

 //Seitenausgabe
 if(!$bSes||empty($sId)){$Et=KAL_TxSessionUngueltig; $Es='Fehl';}
 elseif(!$Et){$Et=KAL_TxNnUebersicht; $Es='Meld';}
 $X=' <p class="kal'.$Es.'">'.fKalTx($Et).'</p>';

 if(KAL_NaviOben>0||KAL_NaviUnten>0) $sNavig=fKalNavigator($nStart,count($aTmp),$sQ,'&amp;kal_Aktion=nbenachr'.KAL_Session);
 if(KAL_NaviOben) $X.="\n".$sNavig;
 ...
 $t='&nbsp;<a href="'.KAL_Self.'?'.substr(KAL_Query.'&amp;kal_Aktion=nbenachr'.KAL_Session.$v,5).'">'.$t.'</a>';
 ...
 foreach($aKalDaten as $a){
  ...
  $sZl="\n".'  <div class="kalTbLst"><span class="kalTbLst">'.fKalTx(KAL_TxLoeschen).'</span><input class="kalCheck" type="checkbox" name="kal_L_'.$s.'#'.$sId.'" value="1"'.(isset($aLsch[$s.'#'.$sId])?' checked="checked"':'').'></div>';

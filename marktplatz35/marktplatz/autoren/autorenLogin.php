...
if($_SERVER['REQUEST_METHOD']=='POST'){
 if(isset($_POST['login'])){
  ...
  $sPass=str_replace('/',':',stripslashes(stripslashes(strip_tags(trim($_POST['pass'])))));
  if(strtolower($sName)==AM_Author&&$sPass==fMpDeCode(AM_AuthPass)){//Zugangsdaten OK
...
echo fSeitenKopf('Anmeldung');

if($_SERVER['REQUEST_METHOD']!='POST'){//GET
 if(!isset($_SESSION['Id'])||$_SESSION['Id']!=md5($sId)){
  echo '<p class="admMeld" style="text-align:center">Weisen Sie sich als berechtigter Autor aus.</p>';
  echo fLoginForm();
 }else{

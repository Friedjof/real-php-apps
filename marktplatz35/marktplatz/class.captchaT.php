...
/*--(c) J. Hummel------------------------------------------------------------
 ...
 function Test($sAnswer,$sPublic,$sQuestion){ //Antwort pruefen
  ...
  if(strlen($sAnswer)&&$sPublic==md5($this->Salt.'*'.strtolower(MP_Zeichensatz!=2?$sAnswer:iconv('UTF-8','ISO-8859-1',$sAnswer)))){ //richtig
 ...
 function Delete(){ //Captcha loeschen
  ...
  if(strlen($sAnswer)&&$sPublic==md5($this->Salt.'*'.strtolower($sAnswer))){ //richtig
   if(file_exists($this->KeyFile)){
    $aC=file($this->KeyFile); $sCsv=''; $sRefTime=strval(time()-900);
    if(is_array($aC)&&($nCnt=count($aC))){
     for($i=0;$i<$nCnt;$i++){
      $sLn=rtrim($aC[$i]);

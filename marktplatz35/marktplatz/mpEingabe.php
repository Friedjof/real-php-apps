...
function fMpSeite(){
 ...
 $aW=array(); $aUpl=array(); $aFehl=array(); $bOK=false;
 ...
 if(in_array('u',$aMpFT)&&MP_NEingabeLogin){ //Login zwingend noetig
 ...
 if($_SERVER['REQUEST_METHOD']!='POST'){ //GET
  $aW[1]=fMpAnzeigeDatum($sAblaufMax);
  for($i=2;$i<$nFelder;$i++){$t=$aMpFT[$i]; // Vorbelegung
   if($t=='t'||$t=='m'||$t=='o') if($s=(isset($aMpAW[$i])?$aMpAW[$i]:'')){
    if($t=='m') $s=str_replace('\n ',"\n",$s); $s='#'.$s; $p=0;
    while($p=strpos($s,'{',$p)){ //Nutzerdatenfelder
     if($q=strpos($s,'}',$p)){
      $v=''; if(isset($aNF)) if($k=array_search(substr($s,$p+1,$q-($p+1)),$aNF)) $v=(isset($aN[$k-1])?$aN[$k-1]:'');
      $s=substr_replace($s,$v,$p,$q-$p+1);
     }else $s=substr_replace($s,'',$p,1);
    }
    $aW[$i]=substr($s,1);
  }}
  ...
  $sZ=''; $sF=''; $sFehl=''; $sSta=(isset($_POST['mp_Sta'])?sprintf('%0d',$_POST['mp_Sta']):(MP_Direkteintrag?'1':'2'));
  ...
  for($i=1;$i<$nFelder;$i++) //mp_Oh: hochgeladene  mp_Oa: alte
   $aOh[$i]=(isset($_POST['mp_Oh'.$i])?$_POST['mp_Oh'.$i]:'');
  for($i=1;$i<$nFelder;$i++) if($aMpEF[$i]||$i==1){ //alle Eingabefelder
   ...
   $v=date('Y-m-d H:i'); $s=fMpAnzeigeDatum($v).date(' H:i');
   if(!MP_SQL) $sZ.=';'.$v;
   else{$sZ.=',"'.$v.'"'; $sF.=',mp_'.$i;}
  }elseif(!MP_SQL) $sZ.=';';
  ...
  if($sFehl==''){ //alles OK, eintragen
   if(count($aFehl)==0){
    ...
    if(!MP_SQL){ //ohne SQL
     $aD=file(MP_Pfad.MP_Daten.$sSegNo.MP_Inserate); $nSaetze=count($aD); $aTmp=array(); $bVmkOK=MP_Direkteintrag;
     ...
     $sMlTx.=strtoupper(MP_TxNummer).': '.(MP_NummerMitSeg?$sSegNo.'/':'').sprintf('%0'.MP_NummerStellen.'d',$nId);
     $aD=array(); $s='Nummer_'.$nId.';online'; for($i=1;$i<$nFelder;$i++) $s.=';'.$aMpFN[$i]; $aD[0]=$s."\n";
...
<script type=\"text/javascript\">
 ...
 $X.="\n".'<form class="mpEing" name="mpEingabe" action="'.fMpHref('eingabe','','','').'" onsubmit="return formSend()" enctype="multipart/form-data" method="post"><input type="hidden" name="mp_Dmy" value="xx" />'.rtrim("\n".MP_Hidden);
 ...
 for($i=1;$i<$nFelder;$i++) if($aMpEF[$i]||($i==1&&$aMpPF[1])){
  $t=$aMpFT[$i]; $v=(isset($aW[$i])?$aW[$i]:''); //Feldinhalt
  ...
  case 'd': //Datum
   if($aMpEF[$i])
    $X.=$sZ.'<input class="mpEing" style="width:7em;" type="text" name="mp_F'.$i.'" value="'.$v.'" maxlength="10" /> <span class="mpMini">'.fMpTx(MP_TxFormat).' '.fMpDatumsFormat().($i!=1?'':', '.fMpTx(MP_TxMaximal).'&nbsp;'.fMpAnzeigeDatum($sAblaufMax)).'</span></div>';
   else $X.=fMpAnzeigeDatum($sAblaufMax).'<input type="hidden" name="mp_F'.$i.'" value="'.fMpAnzeigeDatum($sAblaufMax).'" />';
   break;
  case '@': $X.=fMpAnzeigeDatum(date('Y-m-d')).date(' H:i'); break; //EintragsDatum
  case 'z': //Zeit
   $X.=$sZ.'<input class="mpEing" style="width:7em;" type="text" name="mp_F'.$i.'" value="'.$v.'" maxlength="5" /> <span class="mpMini">'.fMpTx(MP_TxFormat.' '.MP_TxSymbUhr).'</span></div>';
   break;
  case 'j': case 'v': //Ja/Nein
   ...
   break;
  case 'w': //Waehrung
   $X.=$sZ.'<input class="mpEing" style="width:7em;" type="text" name="mp_F'.$i.'" value="'.$v.'" maxlength="16" /> '.MP_Waehrung.'</div>';
   break;
  case 'n': case 'r': case '1': case '2': case '3': case 'o': //Zahlen
   $X.=$sZ.'<input class="mpEing" style="width:7em;" type="text" name="mp_F'.$i.'" value="'.$v.'" maxlength="'.($aMpEL[$i]>0?$aMpEL[$i]:'12').'" />'.($t!='o'?'':' <span class="mpMini">'.(MP_PLZLaenge>0?MP_PLZLaenge.' '.fMpTx(MP_TxStellen):'').'</span>').'</div>';
   break;
  case 'b': //Bild
   $X.=$sZ.'<input class="mpEing" type="file" name="mp_Up'.$i.'" onchange="loadImgFile(this)" accept="image/jpeg, image/png, image/gif" /></div>'; $bMitBild=true;
   ...
   break;
  case 'f': //Datei
   $X.=$sZ.'<input class="mpEing" type="file" name="mp_Up'.$i.'" onchange="loadDatFile(this)" /></div>';
   if($v) $X.="\n".'   <div class="mpNorm" style="float:left;"><input class="mpCheck" type="checkbox" name="mp_Dl'.$i.'" value="1" /><input type="hidden" name="mp_F'.$i.'" value="'.$v.'" /><input type="hidden" name="mp_Oh'.$i.'" value="'.(isset($aOh[$i])?$aOh[$i]:'').'" /> <span class="mpMini">'.$v.' '.fMpTx(MP_TxLoeschen).'</span></div>';
   ...
   break;
  case 'x':
   $X.=$sZ.'<div style="text-align:right;float:right;padding-top:2px;"><a href="'.$sRelPfad.(MP_GMapSource=='O'?'openstreet':'google').'map.php?'.$i.($v?','.$v:'').'" target="geown" onclick="geoWin(this.href);return false;"><img src="iconAendern.gif" width="12" height="13" border="0" title="Koordinaten bearbeiten" alt="Koordinaten bearbeiten"></a></div><div style="margin-right:18px;"><input class="mpEing" style="" type="text" name="mp_F'.$i.'" value="'.$v.'" maxlength="255" /></div></div>';
   break;
  case 'u': // Benutzername
   $X.=$sZ.'<input class="mpEing" style="width:7em;" type="text" name="mp_F'.$i.'" value="'.sprintf('%04d',substr(MP_Session,0,4)).'" readonly="readonly" /></div>';
   break;
  case 'p': // Passwort
   $X.=$sZ.'<input class="mpEing" style="width:12em;" type="password" name="mp_F'.$i.'" value="'.$v.'" maxlength="'.($aMpEL[$i]>0?$aMpEL[$i]:'16').'" /> <span class="mpMini">'.fMpTx(MP_TxPassRegel).'</span></div>';
   break;
  }
  if($i>1&&($v=$aMpET[$i])) $X.="\n".'   <div class="mpNorm"><span class="mpMini">'.str_replace('`,',';',fMpTx($v)).'</span></div>'; // Eingabehilfe
  $X.="\n".'  </div>'."\n".' </div>';
  if(isset($aMpTZ[$i])&&$aMpTZ[$i]>'0'){ if(++$nFarb>2) $nFarb=1;

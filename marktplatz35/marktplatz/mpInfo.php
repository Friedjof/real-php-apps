...
function fMpSeite(){
 ...
 $Meld=''; $MTyp='Fehl'; $H=''; $U=''; $sHid=''; $sId=''; $sInf=''; $sBtr=''; $sTxt=''; $sVon=''; $sEml='';
 $bInf=true; $bBtr=true; $bTxt=true; $bVon=true; $bEml=true; $bCap=true; $bOK=true; $bOkD=false; $bDo=true; $bDSE1=false; $bDSE2=false; $bErrDSE1=false; $bErrDSE2=false;
 ...
 if($_SERVER['REQUEST_METHOD']!='POST'){ //GET
  if(isset($_GET['mp_Nummer'])){
   $sId=sprintf('%d',$_GET['mp_Nummer']); $Meld=MP_TxInfoMeld; $MTyp='Meld'; reset($_GET);
   foreach($_GET as $k=>$v) if(substr($k,0,3)=='mp_') $sHid.='<input type="hidden" name="'.$k.'" value="'.$v.'" />'."\n";
   $sBtr=MP_TxInfoBtr; $sTxt=str_replace('#A',fMpUrl(),str_replace('#S',MP_SegName,str_replace('\n ',"\n",MP_TxInfoTxt)));
  }
  ...
  if(isset($_POST['mp_Nummer'])){
   ...
   foreach($_POST as $k=>$v) if(substr($k,0,3)=='mp_'&&substr($k,3,1)!='i'&&substr($k,3,3)!='Cap'&&substr($k,3,3)!='DSE'){
    ...
    if($k=='mp_iInf'){
     if($sInf=$s){
      if(!fMpIsEMailAdr($s)){$bInf=false; $bOK=false;}
     ...
     }elseif(MP_InfoAbsPflicht){$bEml=false; $bOK=false;}
   }}
   if(MP_InfoDSE1) if(isset($_POST['mp_DSE1'])&&$_POST['mp_DSE1']=='1') $bDSE1=true; else{$bErrDSE1=true; $bOK=false;}
   if(MP_InfoDSE2) if(isset($_POST['mp_DSE2'])&&$_POST['mp_DSE2']=='1') $bDSE2=true; else{$bErrDSE2=true; $bOK=false;}
 }}

 $nSeite=(isset($_GET['mp_Seite'])?(int)$_GET['mp_Seite']:1); $aR=array(); $sIntervallAnfang=date('Y-m-d');
 if($sId>''){ //Inseratedaten holen
  ...
  if(MP_InfoNDetail) $aMpDF=$aMpND; $nFarb=1; //Inseratedetails aufbereiten
   ...
   if($aMpDF[$i]>0&&$t!='p'&&$t!='c'&&substr($aMpFN[$i],0,5)!='META-'&&$aMpFN[$i]!='TITLE'){
    if($s=str_replace('`,',';',$aR[$i])){$u=$s;
     switch($t){
      ...
      case 'm': if(MP_InfoMitMemo){$s=fMpBB(fMpDt($s)); $u=@strip_tags($s);} else{$s=''; $u='';} break; //Memo
       ...
       if($nId=(int)$s){
        ...
        if(!MP_SQL){ //Textdaten
         ...
         for($j=1;$j<$nSaetze;$j++) if(substr($aD[$j],0,$p)==$v){
          ...
          if(!$s=$aN[MP_NutzerInfoFeld]) $s=MP_TxAutorUnbekannt; elseif(MP_NutzerInfoFeld<5&&MP_NutzerInfoFeld>1) $s=fMpDeCode($s);
          break;
         }
        }elseif($DbO){ //SQL-Daten
         if($rR=$DbO->query('SELECT * FROM '.MP_SqlTabN.' WHERE nr="'.$nId.'"')){
          $aN=$rR->fetch_row(); $rR->close();
          if(is_array($aN)){array_splice($aN,1,1); if(!$s=$aN[MP_NutzerInfoFeld]) $s=MP_TxAutorUnbekannt;}
  ...
  $H.="\n</div>\n";
  $sLnk=(MP_InfoLink==''?fMpLref('detail','',$sId,(MP_DetailPopup?'&amp;mp_Popup=1':'')):MP_InfoLink.(!strpos(MP_InfoLink,'?')?'?':'&amp;').'mp_Aktion=detail&amp;mp_Segment='.MP_Segment.'&amp;mp_Nummer='.$sId.(MP_DetailPopup?'&amp;mp_Popup=1':'').MP_Query);
  if(strpos($sLnk,'ttp')!=1||strpos($sLnk,'://')==false) $sLnk=fMpUrl().$sLnk;
  if($_SERVER['REQUEST_METHOD']=='POST'){ //Info versenden
   if($bOK&&!empty($sInf)){
    $Ht=str_replace("\n\n","\n",htmlspecialchars($sTxt,ENT_COMPAT,'ISO-8859-1'));
    if($nP=strpos($Ht,'http://')){
     if(!$nE=strpos($Ht,' ',$nP+1)) $nE=strlen($sHt);
     $Ht=substr_replace($Ht,'</a>',$nE,0);
     $Ht=substr_replace($Ht,'<a href="'.substr($Ht,$nP,$nE-$nP).'" target="info">',$nP,7);
    }
    if($nP=strpos($Ht,'https://')){
     if(!$nE=strpos($Ht,' ',$nP+1)) $nE=strlen($sHt);
     $Ht=substr_replace($Ht,'</a>',$nE,0);
     $Ht=substr_replace($Ht,'<a href="'.substr($Ht,$nP,$nE-$nP).'" target="info">',$nP,8);
    }
    $Ht='<!DOCTYPE html>
<html>
<head>
...
<body class="mpEMail">
<p>'.str_replace("\n","</p>\n<p>",$Ht).'</p>
<p>'.trim(htmlspecialchars($sVon,ENT_COMPAT,'ISO-8859-1').' '.$sEml).'</p>
...
<script type=\"text/javascript\">
 ...
 $X.="\n".'<form class="mpForm" action="'.fMpHref('info',(MP_Popup?'':$nSeite),$sId,(MP_Popup?'&amp;mp_Popup=1':'')).'" method="post">'.rtrim("\n".$sHid).rtrim("\n".MP_Hidden);
 $X.="\n".'<div class="mpTabl">';
 $X.="\n".'<div class="mpTbZl1"><div class="mpTbSp1">'.fMpTx(MP_TxEmpfaenger).'</div><div class="mpTbSp2"><div class="mp'.($bInf?'Eing':'Fhlt').'"><input class="mpEing" type="text" name="mp_iInf" value="'.fMpTx($sInf).'" /></div></div></div>';
 ...
 if(strlen(MP_TxInfoAbsender)>0) $X.="\n".'<div class="mpTbZl2"><div class="mpTbSp1">'.fMpTx(MP_TxInfoAbsender).'</div><div class="mpTbSp2"><div class="mp'.($bEml?'Eing':'Fhlt').'"><input class="mpEing" type="text" name="mp_iEml" value="'.fMpTx($sEml).'" /></div></div></div>';
 $X.="\n".'<div class="mpTbZl1"><div class="mpTbSp1">'.fMpTx(MP_TxInfoLink).'</div><div class="mpTbSp2"><textarea class="mpEing" readonly="readonly">'.fMpTx($sLnk).'</textarea></div></div>';

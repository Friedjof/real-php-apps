...
<title>Inserateeingabe:: OpenStreetMaps - Koordinaten bestimmen</title>
<style type="text/css">
...
<li>entweder durch PLZ-Eingabe oder Ortseingabe im Suchfeld direkt in der Karte</li>
...
<script type="text/javascript">
 ...
 var mbUrl='https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token=<?php echo MP_SMapCode?>';
 ...
 var bDrag=true; if(<?php echo(MP_SMap2Finger?'true':'false');?>) bDrag=!L.Browser.mobile;
 var map=L.map('Karte',{center:[<?php echo $nBr?>,<?php echo $nLn?>],zoom:<?php echo $nZm?>,<?php if(MP_SMap2Finger){ ?>dragging:bDrag,tap:bDrag,<?php } ?>scrollWheelZoom:false,layers:[osm]});
 var osmGeocoder=new L.Control.OSMGeocoder({placeholder:'Ort suchen...'}); map.addControl(osmGeocoder);
 if(<?php echo (MP_SMapTypeControl?'true':'false')?>){var baseLayers={'Karte':osm,'Satellit':sat}; var layerControl=L.control.layers(baseLayers).addTo(map);}
 var marker=L.marker([<?php echo $nPb?>,<?php echo $nPl?>],{opacity:0.7<?php if(MP_TxGMapOrt){?>,title:'<?php echo iconv('ISO-8859-1','UTF-8',MP_TxGMapOrt)?>'<?php }?>}).addTo(map);

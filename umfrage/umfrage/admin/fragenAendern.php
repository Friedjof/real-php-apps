...
echo fSeitenKopf('Frage überarbeiten','<script src="eingabe.js" type="text/javascript"></script>','UFe');
...
$sMs2=''; $bCh=false; $nAntwAnzahl=max(20,ADU_AntwortZahl); $aAw=array('*'); for($i=1;$i<=$nAntwAnzahl;$i++) $aAw[$i]='';
$Pt1=''; $Pt2=''; $Pt3=''; $Pt4=''; $Pt5=''; $Pt6=''; $Pt7=''; $Pt8=''; $Pt9='';
if($_SERVER['REQUEST_METHOD']=='POST'){
 $sNd=''; $bDo=false;
 if(!$nNr=(isset($_POST['FNr'])?(int)$_POST['FNr']:'')) $sNd=', Frage-Nr.'; $ONr=$_POST['ONr']; $OBl=$_POST['OBl'];
 ...
 if(isset($_POST['BlD'])&&$_POST['BlD']){@unlink(UMF_Pfad.'temp/'.$Bld); $Bld='';} //temp Bild loeschen
 $ImN=(isset($_FILES['Bld'])?strtolower(fUmfDateiname(basename($_FILES['Bld']['name']))):''); $ImE=strrchr($ImN,'.');
  ...
  if($i<=1024*UMF_BildKB){
   ...
   if($aIm[0]<=UMF_BildW&&$aIm[1]<=UMF_BildH){ //direkt speichern
    if(copy($_FILES['Bld']['tmp_name'],UMF_Pfad.'temp/'.$ImN)){
     if($Bld) if($Bld!=$ImN) @unlink(UMF_Pfad.'temp/'.$Bld); $Bld=$ImN; //vorheriges Bild weg
    ...
    if($Src){
     ...
     if(ImageJPEG($Dst,UMF_Pfad.'temp/'.$ImN)){
      if($Bld) if($Bld!=$ImN) @unlink(UMF_Pfad.'temp/'.$Bld); $Bld=$ImN;
     }else $sMs2='<p class="admFehl">'.str_replace('#','temp/'.$ImN,UMF_TxDateiRechte).'</p>';
 ...
 if(!$sMs2) if(!$sNd){ //alles OK
  if(!UMF_SQL){ //Text
   ...
   $sNeu=$nNr.';'.$Onl.';'.$Umf.';'.fUmfTxt($Frg).';'.$Bld.';'.fUmfTxt($Bem).';'.fUmfTxt($B2m);
   ...
   if($nNr==$ONr){if($sNeu!=$sLn){$aD[$i]=$sNeu.NL; $bCh=true;}} //Nummer unveraendert
   else{ //Nummer geaendert
    array_splice($aD,$i,1); $nCnt=count($aD); $bCh=true; $ONr=$nNr; $bSuch=true; $bKorr=false;
    for($i=1;$i<$nCnt;$i++){
     $sLn=rtrim($aD[$i]); $p=strpos($sLn,';'); $n=(int)substr($sLn,0,$p);
     if($bSuch){ //Stelle noch nicht erreicht
      if($n>=$nNr){
       if($n==$nNr){$sLn=($n+1).substr($sLn,$p); $bKorr=true; $nL=$n+1;} else $nL=$n;
   ...
   if($bCh){
    if($f=fopen(UMF_Pfad.UMF_Daten.UMF_Fragen,'w')){
     ...
     $sMeld='<p class="admErfo">Die geänderte Frage wurde gespeichert!</p>';
     if($OBl&&($OBl!=$Bld)) if(strpos($s,';'.$OBl.';')==false){@unlink(UMF_Pfad.UMF_Bilder.$OBl); $OBl='';}
    }else $sMeld='<p class="admFehl">'.str_replace('#',UMF_Daten.UMF_Fragen,UMF_TxDateiRechte).'</p>';
   }else $sMeld='<p class="admMeld">Die Frage '.$nNr.' bleibt unverändert!</p>';
  }elseif($DbO){ //SQL
   $sNeu='aktiv="'.$Onl.'",Umfrage="'.$Umf.'",Frage="'.fUmfSql($Frg).'",Bild="'.$Bld.'",Anmerkung1="'.fUmfSql($Bem).'",Anmerkung2="'.fUmfSql($B2m).'"';
   for($i=1;$i<=$nAntwAnzahl;$i++) $sNeu.=',Antwort'.$i.'="'.fUmfSql($aAw[$i]).'"';
   if($nNr==$ONr){
    if($DbO->query('UPDATE IGNORE '.UMF_SqlTabF.' SET '.$sNeu.' WHERE Nummer='.$ONr)){
     if($DbO->affected_rows==1){
      $sMeld='<p class="admErfo">Die geänderte Frage wurde gespeichert!</p>'; $bCh=true;
      if($OBl&&($OBl!=$Bld)) if($rR=$DbO->query('SELECT COUNT(Nummer) FROM '.UMF_SqlTabF.' WHERE Bild="'.$OBl.'"')){
       $aR=$rR->fetch_row(); $rR->close(); if($aR[0]==0){@unlink(UMF_Pfad.UMF_Bilder.$OBl); $OBl='';}
      }
     }else $sMeld='<p class="admMeld">Die Frage '.$nNr.' bleibt unverändert!</p>';
    }else $sMeld='<p class="admFehl">'.UMF_TxSqlAendr.'</p>';
   }else{
    $aD=array();
    if($rR=$DbO->query('SELECT Nummer FROM '.UMF_SqlTabF.' WHERE Nummer>='.$nNr.' ORDER BY Nummer')){
     $a=$rR->fetch_row(); $nL=$a[0];
     if($nL==$nNr){$aD[]=$nL++; while($a=$rR->fetch_row()){$n=$a[0]; if($n==$nL) $aD[]=$nL++; else break;}}
     $rR->close(); $nCnt=count($aD)-1;
     for($i=$nCnt;$i>=0;$i--) $DbO->query('UPDATE IGNORE '.UMF_SqlTabF.' SET Nummer=Nummer+1 WHERE Nummer='.$aD[$i]);
    }
    if($DbO->query('UPDATE IGNORE '.UMF_SqlTabF.' SET Nummer="'.$nNr.'",'.$sNeu.' WHERE Nummer='.$ONr)){
     if($DbO->affected_rows==1){$sMeld='<p class="admErfo">Die geänderte Frage wurde gespeichert!</p>'; $ONr=$nNr; $bCh=true;}
     else $sMeld='<p class="admFehl">'.UMF_TxSqlEinfg.'</p>';
    }else $sMeld='<p class="admFehl">'.UMF_TxSqlEinfg.'</p>';
   }
  }else $sMeld='<p class="admFehl">'.UMF_TxSqlVrbdg.'</p>';
  if($bCh&&$Bld&&($Bld!=$OBl)) if(copy(UMF_Pfad.'temp/'.$Bld,UMF_Pfad.UMF_Bilder.$Bld)){$OBl=$Bld; @unlink(UMF_Pfad.'temp/'.$Bld);}
  else $sMs2.='<p class="admFehl">'.str_replace('#',UMF_Bilder.$Bld,UMF_TxDateiRechte).'</p>';
 }else $sMeld='<p class="admFehl">Es fehlen Eintragungen bei'.substr($sNd,1).'!</p>';
}else{ //GET
 $nNr=(isset($_GET['nr'])?$_GET['nr']:''); $ONr=$nNr; $sMeld='<p class="admMeld">Überarbeiten Sie die Frage Nummer '.$nNr.'.</p>';
 if(!UMF_SQL){
  $aD=file(UMF_Pfad.UMF_Daten.UMF_Fragen); $nCnt=count($aD);
  for($i=0;$i<$nCnt;$i++){$sLn=$aD[$i]; $p=strpos($sLn,';'); if(substr($sLn,0,$p)==$nNr){$aR=explode(';',rtrim($sLn)); break;}}
 }elseif($DbO){
 ...
 $Onl=$aR[1]; $Umf=$aR[2]; $Bld=$aR[4]; $OBl=$Bld;
 $Frg=str_replace('\n ',"\n",str_replace("\r",'',str_replace('`,',';',$aR[3])));
...
<form name="umfEingabe" action="fragenAendern.php<?php if(isset($_SERVER['QUERY_STRING'])&&($sQ=$_SERVER['QUERY_STRING'])) echo '?'.$sQ?>" enctype="multipart/form-data" method="post">
<table class="admTabl" border="0" cellpadding="3" cellspacing="1">
 <tr class="admTabl">
  <td class="admSpa1">Frage-Nr.</td>
  <td>
   <input type="text" name="FNr" value="<?php echo $nNr?>" size="2" style="width:3em;" />
...
function fUmfBBToolbar($Nam){
 ...
 $X.=NL.'  <td>
   <select class="admTool" name="fra_Col'.$Nam.'" onChange="fCol('."'".$Nam."'".',this.options[this.selectedIndex].value); this.selectedIndex=0;" title="'.UMF_TxBB_O.'">
    <option value=""></option>
 ...
 $X.=NL.'  <td>
   <select class="admTool" name="fra_Siz'.$Nam.'" onChange="fSiz('."'".$Nam."'".',this.options[this.selectedIndex].value); this.selectedIndex=0;" title="'.UMF_TxBB_S.'">
    <option value=""></option>

...
echo fSeitenKopf('neue Frage eingeben','<script src="eingabe.js" type="text/javascript"></script>','UFe');
...
$sMs2=''; $bDo=false; $Frg=''; $FNr=''; $Onl=''; $Umf=''; $Bld=''; $Bem=''; $B2m='';
...
if($_SERVER['REQUEST_METHOD']=='POST'){
 ...
 if(!$sMs2) if(!$sNd){ //alles OK
  ...
  if($bDo&&$Bld>'') if(copy(UMF_Pfad.'temp/'.$Bld,UMF_Pfad.UMF_Bilder.$Bld)) @unlink(UMF_Pfad.'temp/'.$Bld);
...
}else{$sMeld='<p class="admMeld">Tragen Sie eine neue Frage in die Date'.(!$DbO?'i <i>'.UMF_Fragen:'ntabelle <i>'.UMF_SqlTabF).'</i> ein.</p>'; $Pkt='1'; $Onl='1';}//GET
echo $sMeld.$sMs2.NL;
$sOptU=''; for($i=1;$i<=26;$i++) $sOptU.='<option value="'.chr($i+64).'">Umfrage '.chr($i+64).'</option>';
?>

<form name="umfEingabe" action="fragenEingabe.php<?php if(KONF>0)echo'?konf='.KONF?>" enctype="multipart/form-data" method="post">
...
Unsaubere Verschachtelungen der BB-Codes oder fehlende schließende Code-Tags können bei der Darstellung zu Problemen führen.</li>
...
<li>Eventuelle Grafiken, Sounds oder Videos direkt innerhalb des Fragetextes, Antworttextes bzw. Anmerkungstextes in Form von BB-Code oder HTML-Code müssen mit anderen Mitteln als diesem Umfrage-Script am angegebenen Ort hinterlegt werden.</li>
</ul>

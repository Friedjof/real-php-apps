...
/*--(c) J. Hummel------------------------------------------------------------
 ...
 function MakeCaptcha($TCol='',$BCol='',$ImgW=0,$ImgH=0){ //Grafik erzeugen und Namen liefern
  $Res='Err'; $Cod=chr(rand(65,90)).substr(time()+rand(9,13),-4); $Pub=md5($Cod); $this->PublicKey=$Pub;
  if($ImgW==0) $ImgW=$this->ImageW; if($ImgH==0) $ImgH=$this->ImageH;
  if($Img=imagecreatetruecolor($ImgW,$ImgH)){
   ...
   $FSiz=5; if($ImgW<70||$ImgH<18) $FSiz=3; $Offs=round($ImgW/5); $RndX=max(floor($Offs--/2),3); $RndY=max($ImgH-(10+$FSiz),1) ;
   for($i=0;$i<5;$i++) imagestring($Img,$FSiz,$i*$Offs+rand(3,$RndX),rand(1,$RndY),substr($Cod,$i,1),$Col);
   $Nam=substr($Pub,0,$this->NameLength).$this->ImageExt;
   if(imagejpeg($Img,$this->FilePath.$Nam,($ImgW>99&&$ImgH>19?80:96))){
